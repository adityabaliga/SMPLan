...
class Incoming:
    ...
    def fromfile(cls, filename, _unit):
        ...
        for rm in raw_material:
            ...
            if numbers_coll.length <1:
                ...
                numbers = int(float(numbers_list[1]))
            elif "Nos" in weight_list or "nos" in weight_list or "NOS" in weight_list:
                numbers = int(float(weight_list[1]))
            else:
                numbers = 1
    ...
    def smpl_no_list_for_history(cls,smpl_no):
        ...
        smpl_no_lst = []
        with CursorFromConnectionFromPool() as cursor:
            cursor.execute("select smpl_no from incoming where smpl_no = %s order by smpl_no asc",(smpl_no,))
            user_data = cursor.fetchone()
        if user_data:
    ...
    @classmethod
    def get_monthly_total(cls, month, year):
        with CursorFromConnectionFromPool() as cursor:
